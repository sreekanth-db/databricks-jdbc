...
### Fixed
- Fixed batch inserts failing with "Statement is closed" when using SEA client (`useThriftClient=0`) for multi-chunk batches or individual fallback execution. Each batch chunk now uses a fresh statement to avoid server-side statement lifecycle interference.
