...
public class DatabricksPooledConnection implements PooledConnection {
    ...
    public Connection getConnection() throws SQLException {
        ...
        ConnectionHandler handler = new ConnectionHandler(physicalConnection);
        connectionHandler = handler;

        // Use a proxy connection object as a virtual connection, so that we do not close the physical connection
        Connection proxyConnection = (Connection) Proxy.newProxyInstance(getClass().getClassLoader(),
                new Class[]{Connection.class}, handler);
        handler.setVirtualConnection(proxyConnection);
        return proxyConnection;
    }

    /**
     * Instead of declaring a class implementing Connection, use a dynamic proxy to handle all calls through the Connection interface.
     */
     ...
     * through the Statement interfaces. The StatementHandler is required in order to return the proper Connection proxy for the
    ...
    private class StatementHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, @Nullable Object[] args)
            ...
            if (methodName.equals("getConnection")) {
                return conHandler.getVirtualConnection(); // the virtual connection from the connection handler
            }
